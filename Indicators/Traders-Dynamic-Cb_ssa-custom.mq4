...
void manageArrow(int i)
{
   if (ShowArrows)
   {
      ...
      if (trend[i]!=trend[i+1])
      {
         ...
         int period = Period();
         if (trend[i] == 1) {
            drawArrow(i,arrowsUpColor,225,false);
            // Alert(period, " , ", 1);
            switch(period)
            {
               case 1 : GlobalVariableSet("CB_SSA_M1", 1); break; 
               case 5 : GlobalVariableSet("CB_SSA_M5", 1); break; 
               case 15 : GlobalVariableSet("CB_SSA_M15", 1); break; 
               case 30 : GlobalVariableSet("CB_SSA_M30", 1); break; 
               case 60 : GlobalVariableSet("CB_SSA_H1", 1); break;
               case 240 : GlobalVariableSet("CB_SSA_H4", 1); break;
               default: GlobalVariableSet("CB_SSA", 1);
            }
         }
         if (trend[i] == -1) {
            drawArrow(i,arrowsDnColor,226,true);
            // Alert(period, " , ", -1);
            switch(period)
            {
               case 1 : GlobalVariableSet("CB_SSA_M1", -1); break; 
